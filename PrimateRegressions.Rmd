...
lmAcetabDiameter <- lm (Abduction ~ AcetabDiameter, data = pri.dat)
lmAcetabDiameter
#   (Intercept)  AcetabDiameter  
       43.9550          0.2051  

lmPelvicBreadth <- lm (Abduction ~ PelvicBreadth, data = pri.dat)
lmPelvicBreadth
#  (Intercept)  PelvicBreadth  
     45.66525        0.07399  
```
...
```{r}
#femur nsa fta sup acetab diameter
```
...
```{r}
# I got nsa, fta, and sup that week, so yes, they are the same.
```
...
```{r}
#independent x = abduction
#dependent y = NSA
lmbonus <- lm (Abduction ~ NSA, data = pri.dat)
```
