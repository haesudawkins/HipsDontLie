...
range.ext <- max(plush.dat$Extension) - min(plush.dat$Extension)
range.ext
# 32

range.add <- max(plush.dat$Adduction) - min(plush.dat$Adduction)
range.add
# 5

range.abd <- max(plush.dat$Abduction) - min(plush.dat$Abduction)
range.abd
# 3

range.pel <- max(plush.dat$PelvicBreadth) - min(plush.dat$PelvicBreadth)
range.pel
# 1

range.mid <- max(plush.dat$MidthighCircumference) - min(plush.dat$MidthighCircumference)
range.mid
# 0.5

range.thigh <- max(plush.dat$ThighLength) - min(plush.dat$ThighLength)
range.thigh
# 2.5

range.dist <- max(plush.dat$DistanceBetweenKnees) - min(plush.dat$DistanceBetweenKnees)
range.dist
# 7
...
flex_mean <- mean(plush.dat$Flexion)
exte_mean <- mean(plush.dat$Extension)
add_mean <- mean(plush.datAdduction)
abd_mean <- mean(plush.dat$Abduction)
pelvic_mean <- mean(plush.dat$PelvicBreadth)
midthigh_mean <- mean(plush.dat$MidthighCircumference)
thigh_mean <- mean(plush.dat$ThighLength)
dis_mean <- mean(plush.dat$DistanceBetweenKnees)

meanvectors <- c((flex_mean), (exte_mean), (add_mean), (abd_mean), (pelvic_mean), (midthigh_mean), (thigh_mean), (dis_mean))

# Can you combine these mean vectors into a matrix? 

meanmatrix <- matrix(meanvectors, nrow = 4, byrow = TRUE)
print(meanmatrix)
